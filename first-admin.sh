...
#
# Яагаад SQL вэ: бүртгүүлэх дэлгэц гэж байхгүй (`/api/v1/auth/register` байхгүй),
# баримтжсан зам болох control plane нь өөрийн vhost, CONTROL_PLANE_HOST, TOTP
# бүртгэл шаарддаг — эхний хүнийг оруулахын тулд өөр бүтээгдэхүүн босгох хэрэг
# гардаг. Business, Benzin хоёр ч мөн ингэж эхэлсэн.
#
...
stack="$(grep -E '^STACK=' "$APP_DIR/.env" | cut -d= -f2)"; stack="${stack:-gerege_nexus}"

docker exec -i "${stack}_postgres" psql -v ON_ERROR_STOP=1 -U postgres -d platform_db \
...
-- энэ сан дээр аль хэдийн байгаа мөрүүдийн яг тэр хэлбэр.
...
# Шалгалт: бичсэн зүйл нь нэвтэрч чадах эсэх. Cookie-гоор баталгаажсан хүсэлт
# Origin эсвэл Sec-Fetch-Site авчрах ёстой тул header нь сайн дурын биш —
# байхгүй бол 403 нь эрхийн алдаа мэт харагдана, тийм биш.
origin="$(grep -E '^PUBLIC_ORIGIN=' "$APP_DIR/.env" | cut -d= -f2)"
port="$(grep -E '^BACKEND_HOST_PORT=' "$APP_DIR/.env" | cut -d= -f2)"; port="${port:-8082}"
code=$(curl -s -o /dev/null -w '%{http_code}' -X POST "http://127.0.0.1:${port}/api/v1/auth/login" \
